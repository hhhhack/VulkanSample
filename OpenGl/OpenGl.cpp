...
void OpenGl::Mainloop()
{
	...
	Texture texture("../Shader/container2.png");
	texture.Load();
	//Texture texture2("../Shader/awesomeface.png", GL_RGBA);
	//texture2.Load();
	...
	while (!glfwWindowShouldClose(m_pWindow))
	{
		...
		//program1.SetVec3("material.ambient", glm::vec3(1.0f, 0.5f, 0.31f));
		//glActiveTexture(GL_TEXTURE_2D);
		program1.SetInt("material.diffuse", 0);
		program1.SetVec3("material.specular", glm::vec3(0.5f, 0.5f, 0.5f));
		program1.SetFloat("material.shininess", 32.0f);

		program1.SetVec3("light.ambient", 0.2f, 0.2f, 0.2f);
		program1.SetVec3("light.diffuse", 0.5f, 0.5f, 0.5f); // 将光照调暗了一些以搭配场景
		program1.SetVec3("light.specular", 1.0f, 1.0f, 1.0f);
		program1.SetVec3("light.position", lightPos);
		glm::mat4 projection(1.0f);
		projection = glm::perspective(glm::radians(45.0f), float(cnWidth) / cnHeight, 0.1f, 100.0f);
